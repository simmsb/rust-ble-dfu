...
pub fn response_is_type<R: Request>(buf: &[u8]) -> bool {
    if buf.len() < 3 {
        return false;
    }
    ...
    let Some(result) = ResultCode::from_primitive(buf[2]) else {
        return false;
    };

    return matches!(result, ResultCode::Success);
}

pub fn parse_response<R: Request>(buf: &[u8]) -> eyre::Result<Option<R::Response>> {
    // Response format:
    // - Fixed byte 0x60
    // - Request opcode
    ...
    let result: ResultCode = ResultCode::from_primitive(buf[2]).ok_or_else(|| {
        eyre::format_err!("malformed response (invalid result code 0x{:02x})", buf[2])
    })?;
    ...
    match result {
        ...
        ResultCode::ExtError => match buf.get(3) {
            ...
            None => {
                eyre::bail!("malformed response (missing extended error byte)");
            }
        },
        code => {
            return Err(DfuError {
    ...
    let response = R::Response::read_payload(&mut response_bytes)
        .with_context(|| eyre::eyre!("While decoding a {:?}", R::OPCODE))?;

    if !response_bytes.is_empty() {
        eyre::bail!("trailing bytes in response");
    }
